		...
		<table cellpadding="0" cellspacing="0" class="table table-bordered table-hover table-condensed">
			...
			<?php foreach($lista as $linha){?>
				<tr>
					<td><?php 
					if($visualizar){
						echo $this->Html->link($linha['Procedimento']['titulo'], array('action' => 'visualizar', $linha['Procedimento']['id'])); 
					}else{
					...
					<td width="7%" nowrap="nowrap" class="text-center">
						<?php 
							...
							echo $this->Html->link(
								__(""),
								array('action' => 'editar', $linha['Procedimento']['id']),
							...
							echo $this->Form->postLink(
								__(""), 
								array('action' => 'excluir', $linha['Procedimento']['id']), 
								array('class'=>'icon-trash'),
								__('Deseja realmente excluir o registro?', $linha['Procedimento']['id'])
							);
							}
						?>
					</td>
