		...
		<table cellpadding="0" cellspacing="0" class="table table-bordered table-hover table-condensed">
			<thead>
				<tr>
					...
					<th><?php echo $this->Paginator->sort('usuario.nome', 'Responsável'); ?></th>
			...
			<?php foreach($lista as $linha){?>
				<tr>
					<td><?php 
					if($visualizar){
						echo $this->Html->link($linha['Objetivo']['titulo'], array('action' => 'visualizar', $linha['Objetivo']['id'])); 
					}else{
						echo $linha['Objetivo']['titulo'];
					}
					?>&nbsp;</td>
					...
					<td width="7%" nowrap="nowrap" class="text-center">
						<?php 
							...
							echo $this->Html->link(
								__(""),
								array('action' => 'editar', $linha['Objetivo']['id']),
							...
							echo $this->Form->postLink(
								__(""), 
								array('action' => 'excluir', $linha['Objetivo']['id']), 
								array('class'=>'icon-trash'),
								__('Deseja realmente excluir o registro?', $linha['Objetivo']['id'])
							);
							}
						?>
					</td>
