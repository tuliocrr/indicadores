		...
		<table cellpadding="0" cellspacing="0" class="table table-bordered table-hover table-condensed">
			...
			<?php foreach($lista as $linha){?>
				<tr>
					<td><?php 
					if($visualizar){
						echo $this->Html->link($linha['Vinculo']['titulo'], array('action' => 'visualizar', $linha['Vinculo']['id'])); 
					}else{
						echo $linha['Vinculo']['titulo'];
					}
					?>&nbsp;</td>
					...
					<td width="7%" nowrap="nowrap" class="text-center">
						<?php 
							...
							echo $this->Html->link(
								__(""),
								array('action' => 'editar', $linha['Vinculo']['id']),
							...
							echo $this->Form->postLink(
								__(""), 
								array('action' => 'excluir', $linha['Vinculo']['id']), 
								array('class'=>'icon-trash'),
								__('Deseja realmente excluir o registro?', $linha['Vinculo']['id'])
							);
							}
						?>
					</td>
