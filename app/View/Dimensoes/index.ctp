		...
		<table cellpadding="0" cellspacing="0" class="table table-bordered table-hover table-condensed">
			...
			<?php foreach($lista as $linha){?>
				<tr>
					<td><?php 
					if($visualizar){
						echo $this->Html->link($linha['Dimensao']['titulo'], array('action' => 'visualizar', $linha['Dimensao']['id'])); 
					}else{
						echo $linha['Dimensao']['titulo'];
					}
					?>&nbsp;</td>
					<td><?php echo $linha["Dimensao"]["ano"]?></td>
					<?php if($editar || $excluir){?>
					<td width="7%" nowrap="nowrap" class="text-center">
						<?php 
							...
							echo $this->Html->link(
								__(""),
								array('action' => 'editar', $linha['Dimensao']['id']),
							...
							echo $this->Form->postLink(
								__(""), 
								array('action' => 'excluir', $linha['Dimensao']['id']), 
								array('class'=>'icon-trash'),
								__('Deseja realmente excluir o registro?', $linha['Dimensao']['id'])
							);
							}
						?>
					</td>
