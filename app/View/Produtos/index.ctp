		...
		<table cellpadding="0" cellspacing="0" class="table table-bordered table-hover table-condensed">
			<thead>
				<tr>
					...
					<th><?php echo $this->Paginator->sort('perda', 'Perda'); ?></th>
			...
			<?php foreach($lista as $linha){?>
				<tr>
					<td><?php 
					if($visualizar){
						echo $this->Html->link($linha['Produto']['titulo'], array('action' => 'visualizar', $linha['Produto']['id'])); 
					}else{
						echo $linha['Produto']['titulo'];
					}
					?>&nbsp;</td>
					<td><?php echo $linha['Produto']['valor'];?></td>
					<td><?php echo $linha['Produto']['referencia'];?></td>
					<td><?php echo $linha['Produto']['perda'];?></td>

					<td><?php echo $linha["TipoProduto"]["titulo"]?></td>
					<?php if($editar || $excluir){?>
					<td width="7%" nowrap="nowrap" class="text-center">
						<?php 
							...
							echo $this->Html->link(
								__(""),
								array('action' => 'editar', $linha['Produto']['id']),
							...
							echo $this->Form->postLink(
								__(""), 
								array('action' => 'excluir', $linha['Produto']['id']), 
								array('class'=>'icon-trash'),
								__('Deseja realmente excluir o registro?', $linha['Produto']['id'])
							);
							}
						?>
					</td>
