		...
		<table cellpadding="0" cellspacing="0" class="table table-bordered table-hover table-condensed">
			<thead>
				<tr>
					...
					<th><?php echo $this->Paginator->sort('limite_vermelho', 'Limite Vermelho'); ?></th>
					<th><?php echo $this->Paginator->sort('limite_amarelo', 'Limite Amarelo'); ?></th>
			...
			<?php foreach($lista as $linha){?>
				<tr>
					<td><?php 
					if($visualizar){
						echo $this->Html->link($linha['Faixa']['titulo'], array('action' => 'visualizar', $linha['Faixa']['id'])); 
					}else{
						echo $linha['Faixa']['titulo'];
					}
					?>&nbsp;</td>
					<td><?php echo $linha["Faixa"]["descricao"]?></td>
					<?php if($editar || $excluir){?>
					<td width="7%" nowrap="nowrap" class="text-center">
						<?php 
							...
							echo $this->Html->link(
								__(""),
								array('action' => 'editar', $linha['Faixa']['id']),
							...
							echo $this->Form->postLink(
								__(""), 
								array('action' => 'excluir', $linha['Faixa']['id']), 
								array('class'=>'icon-trash'),
								__('Deseja realmente excluir o registro?', $linha['Faixa']['id'])
							);
							}
						?>
					</td>
